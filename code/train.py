            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for video_id, val_data in enumerate(val_loader):
                    ...
                    for i in range(t_step):
                        ...
                        psnr_h = cal_pnsr(sr_img_h, lrgt_img)
                ...
                avg_psnr_h = avg_psnr_h / idx
                avg_psnr_lr = avg_psnr_lr / idx

                logger.info('# Validation # PSNR_Cover: {:.4e}, PSNR_Secret: {:.4e}, PSNR_Stego: {:.4e}'.format(avg_psnr, avg_psnr_h, avg_psnr_lr))
                logger_val = logging.getLogger('val')
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> PSNR_Cover: {:.4e}, PSNR_Secret: {:.4e}, PSNR_Stego: {:.4e}'.format(
                    epoch, current_step, avg_psnr, avg_psnr_h, avg_psnr_lr))
                # tensorboard logger
                if opt['use_tb_logger'] and 'debug' not in opt['name']:
